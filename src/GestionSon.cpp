...
void GestionSon::jouerSon(const std::string &note) {
    Logger::log("[GestionSon] ligne 13 : Requete pour jouer le son de la note " + note);

    // Construction du chemin complet du fichier audio
    std::string cheminFichier = "./sounds/" + note + ".wav";
    Logger::log("[GestionSon] ligne 17 : Chemin du fichier audio utilise : " + cheminFichier);
    ...
    if (resultat != 0) {
        Logger::log("[GestionSon] ligne 25 : Erreur : Impossible de lire le fichier audio : " + cheminFichier, true);
    } else {
        Logger::log("[GestionSon] ligne 27 : Lecture du son terminee avec succes : " + cheminFichier);
    }
}
