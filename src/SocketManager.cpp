...
{
    delete serveur;
    Logger::log("[SocketManager] Ligne 17 : Destructeur appele, serveur supprime.");
}
...
bool SocketManager::initialiserServeur(int port)
{
    ...
    {
        Logger::log("[SocketManager] Ligne 31 : Erreur : Impossible de demarrer le serveur.", true);
        return false;
    }

    Logger::log("[SocketManager] Ligne 35 : Serveur demarre sur le port " + std::to_string(port) + ".");
    return true;
}
...
void SocketManager::attendreConnexion()
{
    while (!clientSocket)
    {
        QCoreApplication::processEvents(); // Traiter les evenements Qt pendant l'attente
    }
...
void SocketManager::envoyerMessage(const QJsonObject &message)
{
    ...
    {
        Logger::log("[SocketManager] Ligne 54 : Erreur : Aucun client connecte pour envoyer le message.", true);
        return;
    }
    ...
    Logger::log("[SocketManager] Ligne 63 : Message envoye : " + data.toStdString());
}
...
QString SocketManager::recevoirMessage()
{
    if (!clientSocket)
    {
        Logger::log("[SocketManager] Ligne 71 : Erreur : Pas de client connecte.", true);
        return QString();
    }
    ...
    {
        Logger::log("[SocketManager] Ligne 83 : Aucun message recu apres delai de 3 secondes.");
        return QString();
    }

    if (clientSocket->state() == QAbstractSocket::UnconnectedState)
    {
        Logger::log("[SocketManager] Ligne 89 : Le client est deconnecte.", true);
        return QString();
    }
    ...
    if (data.isEmpty())
    {
        Logger::log("[SocketManager] Ligne 96 : Avertissement : Message vide recu.");
        return QString();
    }

    Logger::log("[SocketManager] Ligne 100 : Message recu : " + data.toStdString());
    return QString::fromUtf8(data);
}


// Traitement d'un message JSON recu et conversion en map
std::map<std::string, std::string> SocketManager::traiterMessage(const QString &message)
{
    ...
    {
        Logger::log("[SocketManager] Ligne 113 : Erreur : Message JSON invalide.", true);
        return result;
    }
