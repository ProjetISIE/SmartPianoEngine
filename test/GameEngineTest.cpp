...
TEST_CASE("GameEngine Error Handling") {
    ...
    SUBCASE("Unknown Game Mode") {
        Message configMsg("config", {{"game", "unknown"}, {"scale", "c"}, {"mode", "maj"}});
        transport.pushIncoming(configMsg);
    ...
    SUBCASE("MIDI Not Ready") {
        ...
        Message configMsg("config", {{"game", "note"}, {"scale", "c"}, {"mode", "maj"}});
        transport.pushIncoming(configMsg);
    ...
    SUBCASE("Client Quit during Config") {
        ...
        // Should log and continue waiting. We can verify it's still running by sending valid config after.
...
TEST_CASE("GameEngine Game Modes") {
    ...
    SUBCASE("Chord Game") {
        Message configMsg("config", {{"game", "chord"}, {"scale", "c"}, {"mode", "maj"}});
        transport.pushIncoming(configMsg);
    ...
    SUBCASE("Inversed Game") {
        Message configMsg("config", {{"game", "inversed"}, {"scale", "c"}, {"mode", "maj"}});
        transport.pushIncoming(configMsg);
        transport.waitForSentMessage(); // Ack
        transport.pushIncoming(Message("ready"));
        Message challenge = transport.waitForSentMessage();
